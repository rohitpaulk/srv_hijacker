...
try:
    ...
    raise pytest.skip("psycopg2 is not available, skipping test that patches psycopg2", allow_module_level=True)
    ...
    def test_hijack_psycopg2(self, consul_pg_hostname):
        ...
        with psycopg2.connect(dsn) as conn:
            with conn.cursor() as cur:
                ...
                result = cur.fetchone()
                assert result[0] == 1, f"unexpected query result {result[0]}, expected 1"
